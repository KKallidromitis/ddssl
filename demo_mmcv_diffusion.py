...
resize_kornia = VideoSequential(
    kornia.augmentation.Resize((CROP_SIZE,CROP_SIZE)),
    random_apply=False,
    data_format="BCTHW",
    same_on_frame=True)
...
SIZE = 2350   # 1p
LATENT_SIZE = 512 # diffusion
indices = list(range(SIZE))
video_syn = torch.randn(size=(SIZE, 2,LATENT_SIZE), dtype=torch.float, requires_grad=False)
...
LR_D = 0.0001
optimizer_diffusion = Adam(diffusion_model.parameters(),lr=LR_D, weight_decay=0.01)
...
def update_lr(scheduler,base_lr,optimizer):
    new_lr = scheduler.get_lr(base_lr,total_steps,curr_step)
...
for epoch in range(epoches):
    ...
    iter = 0
    curr_step = epoches*total_iter
    total_steps = epoch*total_iter+iter
    new_lr = update_lr(lr_scheduler,LR,optimizer_model)
    #update_lr(lr_scheduler,LR_D,optimizer_diffusion)
    ...
    for batch in dataloader:
        ...
        syn_batch = video_syn[idx].detach().clone().to(device) # N X2 X 512
        syn_batch.requires_grad=True
        real_video = batch['imgs'].to(device)
        real_video_aug,parms = apply_kornia(aug_list,real_video)
        # Train Diffusion
        mmaction_shape = real_video.shape#  B N C T H W
        diffusion_model.train()
        loss_diffusion = diffusion_model.forward_loss(real_video_aug.view(-1,3,*mmaction_shape[-2:]))
        loss_diffusion = DiffusionAPI.forward_loss(diffusion_model,) 
        x_reconst,x_diffusion_reconst,cond,loss_l2,loss_vae,dkl = loss_diffusion['reconst'],loss_diffusion['pred_xstart'],loss_diffusion['cond'],loss_diffusion['l2_loss'],loss_diffusion['vae_loss'],loss_diffusion['dkl']
        #loss_diffusion += 
        #TODO: ADD VAE PROCESS
        loss_diffusion = loss_diffusion['loss']
        optimizer_diffusion.zero_grad()
        loss_diffusion.backward()
        # manual ddp
        average_gradients(diffusion_model)
        ...
        optimizer = SGD([syn_batch,],lr=0.01, momentum=0.9, weight_decay=0.01)
        ...
        vfs_loss_real = torch.FloatTensor([0.0,]).mean()
        for _ in range(INNER_LOOP):
            syn_latent = syn_batch.flatten(0,1) # 2N  X 512
            diffusion_model.eval()
            syn_video_decode = None
            if False: # VFS LOOP
                syn_video_decode = DiffusionAPI.decode_step(diffusion_model,syn_latent,2).view(*mmaction_shape) 
                syn_video_aug,_ = apply_kornia(aug_list,syn_video_decode,parms)
                r_syn = model(syn_video_aug)
                ...
                optimizer.step()
                pass
            else: #first moment matching
                syn_video_decode = DiffusionAPI.decode_step(diffusion_model,syn_latent,2).view(*mmaction_shape) 
                syn_video_aug,_ = apply_kornia(aug_list,syn_video_decode,parms)
                d = cond.shape[-1]
                match_loss = torch.nn.functional.mse_loss(diffusion_model.encode(syn_video_aug.view(-1,3,*mmaction_shape[-2:])).mean(0),cond.mean(0))
                optimizer.zero_grad()
                match_loss.backward()
                # syn_batch.grad.data = syn_batch.grad.data / world_size
                optimizer.step()
        ...
        if iter % 10 == 0 and rank == 0:
            if iter % 50 == 0 :
                ...
                if INNER_LOOP>0:
                    if syn_video_decode is None:
                        with torch.no_grad():
                            syn_video_decode = DiffusionAPI.decode_step(diffusion_model,syn_latent,10).view(*mmaction_shape) 
                    img = syn_video_decode[0,:,:,0].detach().cpu().numpy().transpose(0,2,3,1)
                    wandb_dump_img([img[0],img[1]],'img_syn')
                img = real_video_aug[0,:,:,0].detach().cpu().numpy().transpose(0,2,3,1)
                reconst = x_reconst[0:2].detach().cpu().numpy().transpose(0,2,3,1)
                wandb_dump_img([img[0],img[1],reconst[0],reconst[1]],'img_real')
            wandb.log(
                dict(
                    epoch=epoch,
                    lr=new_lr,
