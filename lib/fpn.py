...
class FCNHead(nn.Module):
    def __init__(self, embed_dim=512, hidden_dim=256, num_layers=3, target_dim=3) -> None:
        super().__init__()
...
class Generator(nn.Module):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.fcn1 = FCNHead(embed_dim=512, hidden_dim=256, num_layers=3, target_dim=3)
        self.fcn2 = FCNHead(embed_dim=512, hidden_dim=256, num_layers=3, target_dim=3)
        self.fpn = FPNHead(embed_dim=512)

    def forward(self,x):

        x0,x1 = self.fpn(x)
        y0 = self.fcn1([x0])[0] # high
        y1 = self.fcn2([x1])[0] # low
        final = y0 + nn.functional.interpolate(y1,(224,224),mode='bilinear')
        return final