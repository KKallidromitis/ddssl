...
class BYOLTrainer():
    def __init__(self, config):
        ...
        if self.gpu==0:
            wandb.init(project="ddssl_randomall",name = self.time_stamp+'_gpu_'+str(self.rank))
    ...
    def resume_model(self, model_path=None):
        if model_path is None and not self.resume_path:
            ...
            if type(checkpoint) is dict:
                ...
                amp.load_state_dict(checkpoint)
                self.logging.info(f"--> Loaded checkpoint '{model_path}' (epoch unknown)")
